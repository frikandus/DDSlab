...
qid_sum <- kdb %>% group_by(severity, patchable) %>% summarise(contador = n())

kable(x = qid_sum, 
      format = "html",
      col.names = c("Severidad", "Parcheo posible", "Total"),
      caption = "Vulnerabilidades ordenadas por severidad y capacidad de parcheo") %>% kable_classic(full_width = FALSE, html_font = "Cambria", font_size=16) 
```
